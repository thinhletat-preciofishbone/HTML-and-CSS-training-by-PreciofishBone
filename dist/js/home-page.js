...
function setNewItemModalInfo(_itemInfo) {
  ...
  if (_itemInfo.type === 'folder') {
    document.getElementById('item-type').setAttribute('data-item-type', 'folder');
  } else {
    document.getElementById('item-type').setAttribute('data-item-type', 'file');
  }
}
...
function activateNavigationMenuOperations() {
  ...
  document.getElementsByClassName('save-new-item')[0].addEventListener('click', () => {
    ...
    let currentFolderData = _view_index__WEBPACK_IMPORTED_MODULE_0__["default"].getCurrentFolderData(); // get the item info
    // -- item name
    ...
    const itemType = document.getElementById('item-type').getAttribute('data-item-type'); // create item data
    ...
    if (itemType === 'folder') {
      id = 'folder-' + makeid(5);
      name = itemName.value;
      const folderItem = new _classes_index__WEBPACK_IMPORTED_MODULE_1__["default"].Folder(id, name);
      currentFolderData.folders.push(folderItem);
    } else {
      id = 'file-' + makeid(5);
      name = itemName.value;
      let extension = ''; // file does have extension
      ...
      const fileItem = new _classes_index__WEBPACK_IMPORTED_MODULE_1__["default"].File(id, name, extension);
...
/***/ "./src/scripts/components/document-table.ts":
/*!**************************************************!*\
  !*** ./src/scripts/components/document-table.ts ***!
  \**************************************************/
/*! exports provided: default */
...
/* harmony import */ var _components_header__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ../components/_header */ "./src/scripts/components/_header.ts");
/* harmony import */ var _components_document_table__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ../components/document-table */ "./src/scripts/components/document-table.ts");
/* harmony import */ var _view_index__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ../view/index */ "./src/scripts/view/index.ts");





function getFolderIdFromLocalStorage(_directorypath) {
...
Object(_utilities_helper__WEBPACK_IMPORTED_MODULE_0__["default"])(() => {
  ...
  if (!urlParams.has('directory') || urlParams.get('directory') === 'root') {
    _view_index__WEBPACK_IMPORTED_MODULE_3__["default"].renderDirectory();
  } else {
    const folderId = getFolderIdFromLocalStorage(urlParams.get('directory'));
    console.log(folderId);
    _view_index__WEBPACK_IMPORTED_MODULE_3__["default"].renderDirectory(folderId);
  }

  _components_header__WEBPACK_IMPORTED_MODULE_1__["default"].activateOperations();
  _components_document_table__WEBPACK_IMPORTED_MODULE_2__["default"].activateOperations();
});
...
function createItemRecord(_item) {
  ...
  if (_item.extension !== undefined) {
    ...
    td.addEventListener('click', () => {
      ...
      const urlParams = new URLSearchParams(window.location.search);
      window.localStorage.setItem(urlParams.get('directory'), _item.id);
    });
  }
